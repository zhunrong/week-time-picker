  ...
  render() {
    return (
      <div class="day-time-picker">
        <div class="label">{this.day.label}</div>
        <ul title={this.title}>
          {this.day.data.map((item, col) => {
            return (
              <li
                class={{ active: item.active, selected: item.selected }}
                key={col}
                data-row={this.day.index}
                data-col={col}
        ...
        <div class="clear">
          <icon class={{ disabled: !this.title }} vOn:click_native={this.clear} />
        </div>
      </div>
    );
  },
